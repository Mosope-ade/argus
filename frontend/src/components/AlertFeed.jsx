...
export default function AlertFeed({ wsMessages = [] }) {
  const [alerts, setAlerts] = useState([]);
  const [investigating, setInvestigating] = useState({});
  // Track which alert IDs we've already added to prevent duplicates
  ...
  useEffect(() => {
    const loadAlerts = async () => {
      try {
        ...
        const initial = data.alerts || [];
        initial.forEach((a) => seenIds.current.add(a.alert_id));
        setAlerts(initial);
      } catch (err) {
        console.error(err);
      }
  ...
  useEffect(() => {
    ...
    // Only process new_alert type messages and only if we haven't seen this ID
    if (
      latest?.type === "new_alert" &&
      latest.alert?.alert_id &&
      !seenIds.current.has(latest.alert.alert_id)
    ) {
      seenIds.current.add(latest.alert.alert_id);
  ...
  return (
    <div className="h-full overflow-y-auto bg-[#080c12]">
      <div className="p-4 border-b border-[#1a2535]">
        <div className="text-[#e2e8f0] font-mono">ALERT FEED</div>
      </div>
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-[#ff3c5a]">●</span>
              ...
              <button
                onClick={() => handleInvestigate(alert.alert_id)}
